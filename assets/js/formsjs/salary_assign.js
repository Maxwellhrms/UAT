$(document).ready(function () {
    $("form#sal_assign_form").submit(function (e) {
        ...
        if (sal_month_year == "") {
            ...
            $('#sal_month_year_error').html("Please Select Month & Year");
            return false;
        } else {
            $('#sal_month_year_error').html("");
        }

        let unpaid_empids = $("#unpaid_empids").val();
        //--------------NEW BY SHABABU(17-06-2022)
            var data_confirm = confirm("Before submit please check the following things you have cleared or not ie 1)LEAVES ADJUSTMENT 2) TDS AMOUNT 3) INCENTIVE AMOUNT 4) MISC.Income and then process salary");
            if(!data_confirm){
                return false;
            }
        ...
        $.ajax({
				...
				// 		return false;
                        ...
                        if (parse_data.length > 0) {
                            $("#unpaid_empids").empty();
                            $("#unpaid_empids").append('<option value="">-- Select Unpaid Employees --</option>');
                            for (index in parse_data) {
                                ...
                                var auth_resign_status = auth_data['mxemp_emp_resignation_status'];           
                                
                                var opt_data = auth_emp_code + " - " + auth_emp_name + " - " + auth_desg_name + " -  ( " +auth_resign_status + " )";
                                var opt_val = auth_emp_code;
                                // alert(opt_val);
                                
                                $("#unpaid_empids").append('<option value="' + opt_val + '">' + opt_data + '</option>');
                            }
