...
<div id="attendauthleavelist"> </div>
...
e.preventDefault();  
        ...
        // if(empid == ''){
        //     empid = '<?php //echo $this->session->userdata('user_id'); ?>';
        // }
        if(mnth.length == 1){
            var month1 = 0 + mnth;
        }else{
        ...
        // alert(month1);
        var month_year= year + - +month1;
        // alert(month_year);
        var mainurl = baseurl+'mobileapidisplay/attendanceleavelist';
        $.ajax({
            url: mainurl,
            type: "post",
            async: false,
            data: {'month_year': month_year,'companyid': company_id, 'divisonid': div_id,'stateid':state_id,'branchid':branch_id,'employeeid':empid,'filter':'1','finalhraccept':'0','approvstatus':approvstatus},
            success: function (data) {
             console.log(data);
               $("#attendauthleavelist").html(data);
